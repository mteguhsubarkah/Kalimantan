...
const char* mqtt_server = "syergie-ann.local";

//IPAddress mqtt_server(10, 48, 20, 36);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,"steer2_kp") == 0){
    ...
    Kp_baru = (String(String(data_in[0])+String(data_in[1])+String(data_in[2])+String(data_in[3]))).toInt();
    sprintf(buf_Tx,"\t%dS\t\n",Kp_baru);
    mySerial.print(buf_Tx);
  }
  if(strcmp(topic,"steer2_ki") == 0){
    client.publish("confirm_ki2","Received");
    Ki_baru = (String(String(data_in[0])+String(data_in[1])+String(data_in[2])+String(data_in[3])+String(data_in[4])+String(data_in[5]))).toInt();
    sprintf(buf_Tx,"\t%dS\t\n",Ki_baru);
    mySerial.print(buf_Tx);
  }
  if(strcmp(topic,"steer2_kd") == 0){
    client.publish("confirm_kd2","Received");
    Ki_baru = (String(String(data_in[0])+String(data_in[1])+String(data_in[2])+String(data_in[3]))).toInt();
    sprintf(buf_Tx,"\t%dS\t\n",Ki_baru);
    mySerial.print(buf_Tx);
  }
  else if(topic[0]=='M'){
